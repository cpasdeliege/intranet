...
<%
List<Rue> listeRues = (List<Rue>)request.getAttribute("listeRues");
%>
<body class="body">
		...
		<td>
			<table align="center">
				<tr>
					...
					<td class="menu_rues">
						<a href="index.rues?param=A08">Grivegn�e</a>
					</td>
					<td class="menu_rues">
				...
				<tr>
					...
					<td>
						<table cellspacing="0">
							...
							<% } %>
								<td class="liste_rues"><%= rue.getCodeRue() %></td>
								<td class="liste_rues"><a target="_blank" href="http://maps.google.fr/maps?f=q&hl=fr&q=<%= rue.getTypeRue()+"+"+rue.getNomRue()+"+"+rue.getCodePostal()+"+Liege" %>"><img  style="border: none;" title="afficher dans Google Maps" width="20px" alt="map" src="images/googleMap.gif">  </a></td>
								<td class="liste_rues"><%= rue.getNomRue() %> </td>
								<td class="liste_rues"><%= rue.getTypeRue() %></td>
								<td class="liste_rues"><%= rue.getCodePostal() %></td>
								<td class="liste_rues"><%= rue.getAntenne() %></td>
								<td class="liste_rues"><%= rue.getCodeAntenne() %></td>
								<td class="liste_rues" width="80px"><%= rue.getRemarque() %></td>
							</tr>
							<%
								}
							%>
